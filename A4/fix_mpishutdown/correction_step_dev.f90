...
use globals
implicit none
integer :: k,l,piv(N+1),lapack_flag,allocate_err,apple
double precision :: z(N+1),z0(N+1),T(N+1),eps,re(N+1),r(N+1),y(N+1)
double precision , allocatable, dimension (:,:) :: J 


! Uncomment next 2 lines to run test when there is an allocation error
apple = 70000000
allocate(J(apple+1, apple+1), stat=allocate_err)
