...
! End initializations
!do i=2,numprocs-1
!   call MPI_RECV(init_fail,1,MPI_LOGICAL,i,MPI_ANY_TAG,MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr)
!end do 
...
inner: do while(working)
        ...
        willconv=0d0
         ...
         do i=1,numprocs-1
            call MPI_RECV(init_fail,1,MPI_LOGICAL,i,MPI_ANY_TAG,MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr)
            print *, "init_fail: ", init_fail
            if (init_fail.eq.(.true.)) then
               print *, "FAILED>>>" 
               fail=.false.
            end if
         end do
      ...
      ! If only one approximate solution is left then ...
         if(numprocs-1-nkill.eq.1) then
            ...
            print *, "~~~"
            print *, "conv: ", conv, "willconv: ", willconv
            k=minloc(r,1)
